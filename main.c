...
static int check_offsets(uint64_t seed)
{
    ...
    xSkipN(&x3, 1);
    float off0a = fabsf((xNextLong(&x3) >> 32 & 0xFFFFFF) * (1.0f / 16777216.0f) - 0.5f);
    xlo = xNextLong(&x2);
    xhi = xNextLong(&x2);
    x3.lo = xlo ^ 0x36d326eed40efeb2;
    x3.hi = xhi ^ 0x5be9ce18223c636a;
    xSkipN(&x3, 1);
    float off0b = fabsf((xNextLong(&x3) >> 32 & 0xFFFFFF) * (1.0f / 16777216.0f) - 0.5f);
    return off0a + off0b < 0.05;
}
...
static void lattice(ClimateNoises *n, uint64_t seed, double max_a, int sign, int x, int z)
{
